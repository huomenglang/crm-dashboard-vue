...
import type { WarehouseProps } from "./warehouse";
import RoundButton from "@/components/base/button/RoundButton.vue";

const props = defineProps<{
  open: boolean;
  isEditing: boolean;
  initialValues?: Partial<WarehouseProps>;
}>();
...
<template>
  <Modal
    :open="props.open"
    :title="props.isEditing ? 'Edit Warehouse' : 'Create Warehouse'"
    :footer="null"
    destroyOnClose
    @cancel="emit('close')"
  >
    ...
    >
      ...
      <Form.Item label="Location" name="location">
        <Field name="location" v-slot="{ field, errorMessage }">
          <Input v-bind="field" placeholder="Enter location" />
          <div v-if="errorMessage" class="text-red-500 text-sm">
            {{ errorMessage }}
          </div>
        </Field>
